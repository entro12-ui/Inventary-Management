...
class Business(Base):
    ...
    default_min_stock = Column(Integer, nullable=False, default=10)
    business_type = Column(String(30), nullable=False, default="general")
    ...
    proformas = relationship("Proforma", back_populates="business", cascade="all, delete-orphan")
...
class Product(Base):
    ...
    sale_unit = Column(String(20), nullable=False, default="piece")
    sale_unit_custom = Column(String(50), nullable=True)
...
class Proforma(Base):
    """Proforma invoice / quotation / open-tender bid (does not deduct stock)."""
    ...
    proforma_number = Column(String(100), unique=True, nullable=False)
    # quotation | tender_bid
    document_type = Column(String(30), nullable=False, default="quotation")
    ...
    # Issuer snapshot (from business at create/update)
    issuer_name = Column(String(200), nullable=False)
    issuer_address = Column(Text)
    issuer_city = Column(String(100))
    issuer_country = Column(String(100))
    issuer_phone = Column(String(50))
    issuer_email = Column(String(200))
    issuer_logo_url = Column(String(500))
    issuer_tin = Column(String(100))

    # Client / company the proforma is sent to
    client_company_name = Column(String(200), nullable=False)
    client_contact_name = Column(String(200))
    client_address = Column(Text)
    client_phone = Column(String(50))
    client_email = Column(String(200))
    client_tin = Column(String(100))

    # Open tender fields
    tender_title = Column(String(300))
    tender_reference = Column(String(150))
    tender_closing_date = Column(DateTime(timezone=True), nullable=True)
    ...
    authorized_name = Column(String(200))
    authorized_title = Column(String(200))
    ...
    business = relationship("Business", back_populates="proformas")
    created_by_user = relationship("User", foreign_keys=[created_by])
    items = relationship("ProformaItem", back_populates="proforma", cascade="all, delete-orphan")

    __table_args__ = (
        Index("idx_proforma_business", "business_id"),
        Index("idx_proforma_number", "proforma_number"),
        Index("idx_proforma_status", "status"),
        Index("idx_proforma_issue", "issue_date"),
    )
...
class ProformaItem(Base):
    ...
    # Price snapshot — editable even when linked to a product
    unit_price = Column(Float, nullable=False, default=0.0)
    discount_amount = Column(Float, default=0.0)
    line_total = Column(Float, nullable=False, default=0.0)
    # catalog = from product selling_price; custom = manually edited
    price_source = Column(String(20), default="catalog")
    ...
    __table_args__ = (
        Index("idx_proforma_item_proforma", "proforma_id"),
        Index("idx_proforma_item_product", "product_id"),
    )
